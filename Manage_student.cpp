/*Viết chương trình C++ để giữ các bản ghi và thực hiện các trình phân tích thống kê cho một lớp gồm 20 sinh viên. 
Thông tin của mỗi sinh viên gồm id, tên, tuổi, điểm kiểm tra (hai điểm kiểm tra mỗi kỳ), điểm chuyên cần, điểm thi giữa kỳ, điểm thi cuối kỳ, và tổng điểm.
...
void displaymenu(){
     ...
     cout<<" 4. Quan sat tat ca ban ghi sinh vien"<<endl;
     cout<<" 5. Tinh diem trung binh cua sinh vien da chon"<<endl;
     cout<<" 6. Hien thi sinh vien co tong diem cao nhat"<<endl;
     cout<<" 7. Hien thi sinh vien co tong diem thap nhat"<<endl; 
     cout<<" 8. Tim sinh vien boi ID"<<endl; 
     cout<<" 9. Sap xep cac ban ghi boi tong diem"<<endl; 
}
...
void viewall(struct sinhvien st[], int biendem){
     int i=0;
     cout<<left<<setw(5)<<"ID"<<setw(20)<<"TEN"<<setw(5)<<"GIOI TINH"<<setw(5)<<"DKT1"<<setw(5)
     <<"DKT2"<<setw(5)<<"DCC"<<setw(5)<<"DGK"<<setw(5)<<"DCK"<<setw(5)<<"TONG"<<endl;
     cout<<"==============================================\n";
     while(i<=biendem){
          if(st[i].mssv!=""){
...
void capnhat_banghi(struct sinhvien st[],int biendem){
     ...
     if (index != -1){
          if (column_index == 1){
               ...
               cout<<"Gia tri chi muc khong hop le";
...
void find(struct sinhvien st[], int biendem) {
     ...
     cin>>id;
     int index=search(st,id,biendem);
     if (index != -1){        //hien thi ban ghi da tim thay
